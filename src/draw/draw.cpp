...
void PaintApp::flushPoints()
{
    ...
    {
        int gx = p.x - canvas.originX();
        int gy = p.y - canvas.originY();
        canvas.setGridPixel(gx, gy, color);
    }
    points.clear();
    dirty = true;
}
...
void PaintApp::ensureCanvasCoversView()
{
    ...
    int needL = floorDiv((int)std::floor(camX), CHUNK) * CHUNK;
    int needT = floorDiv((int)std::floor(camY), CHUNK) * CHUNK;
    int needR = ceilDiv((int)std::ceil(viewR), CHUNK) * CHUNK;
    int needB = ceilDiv((int)std::ceil(viewB), CHUNK) * CHUNK;
...
void PaintApp::saveCanvasBMP()
{
    ...
    writeBMPFile(name, fillBuf.data(), w, h);

    std::cout << "Saved " << name << " (" << w << "x" << h << ")" << std::endl;
    saveFlashUntil = SDL_GetTicks() + 1200;
}
